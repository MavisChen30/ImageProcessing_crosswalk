...
# construct the argument parser and parse theommand line arguments
...
# initialize the HOG descriptor along with the list ofata and labels
hog = HOG(orientations=conf["orientations"],pixelsPerCell=tuple(conf["pixels_per_cell"]),
          cellsPerBlock=tuple(conf["cells_per_block"]),normalize=conf["normalize"])
data = []
labels = []

# grab the set of ground-truth images and select apercentage of them for training
...
for (i, trnPath) in enumerate(trnPaths):
    ...
    # imageID = trnPath[trnPath.rfind("10000") + 1:].replace(".jpg", "")

    imageID = os.path.splitext(os.path.basename(trnPath))[0]
    p = os.path.join(conf["image_annotations"], f"{imageID}.mat")
    # print(f"[DEBUG] Looking for annotation file: {p}")
    ...
    # p = "{}/1{}.mat".format(conf["image_annotations"], imageID)
    ...
    # loop over the ROIs
    for roi in rois:
        # extract features from the ROI and update the list ofeatures and labels
...
if not dstPaths:
    print("[ERROR] No distraction image paths found in the specified directory.")
    exit(1)
...
for i in np.arange(0, conf["num_distraction_images"]):
    ...
    if image is None:
        print(f"[WARNING] Unable to load image from path: {dstPaths}")
        continue
    image = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
